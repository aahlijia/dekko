...
# rare enough that naming it is exactly the useful signal (an agent
# once hit "no mapped file" on a real, first-party
...
def _json_dumps(obj: object) -> bytes:
    ...
    files (the provenance sidecar, ``map.json``) where
...
def build_id_table(graph: CallGraph) -> tuple[list[str], dict[str, int]]:
    ...
    other entries (a measured 463.6x duplication factor
...
def _intern_modules(graph: CallGraph, intern: Callable[[str], None]) -> None:
    ...
    Only the edges' ``importer``/``imported`` paths are interned —
    ``ModuleGraph.external``'s *values*
...
def atomic_write_bytes(path: Path, data: bytes) -> None:
    ...
    No synchronization primitive of any
...
def _too_large_summary(
    ...
    useful signal, and an agent otherwise has no way
...
def format_unsupported(provenance: dict | None) -> str | None:
    ...
    (``provenance["too_large"]`` — e.g. a large first-party test
    file), and symlinked files skipped by default to
    avoid double-indexing (``provenance["symlink_excluded"]``).
    Shared by ``dekko status``, ``map_status``,
...
class MapIndex:
    ...
    Attributes:
        ...
        heritage_external_out: Subtype symbol id → heritage clauses
            ...
            per-symbol lookup, not a repo-wide name search. It is
            deliberately not a name-indexed
            ``heritage_externals_by_name`` (the ``externals_by_name``
            shape) — nothing needs a name-indexed heritage-external
            search yet, and a caller-indexed table is what
            ``supertypes`` rendering actually requires.
        heritage_synthetic_tiebreak_count: Repo-wide count of resolved
            heritage edges that rest on the heritage crate-decoy
            tiebreak rather than an
        ...
        heritage_unplaced_subtype_count: Repo-wide count of Rust
            cross-file ``impl Trait for Type`` clauses
        ...
        doc_version: The on-disk document's ``"version"`` field (``1``
            ...
            itself changed underneath them" (a
...
class Freshness:
    ...
    Attributes:
        ...
        spec_stale: True when the map's recorded ``spec_hash`` differs
            ...
            extractor code (a ``dekko serve`` once reported
        ...
        process_outdated: True when a long-lived process found that
            ...
            *installed* code and its sources are unchanged, so it is
            served as-is. Always False in a
...
def _same_version_explanation(fresh: Freshness) -> str:
    ...
    This used to always blame the reader ("this is a
    ...
    truth, and the reason two reports contradicted each other about
    which spec was current. Now it only says what is proven.
    """
...
def describe_version_stale(fresh: Freshness) -> str:
    ...
    This names the differentiator explicitly using
...
def load_map(root: Path) -> MapIndex | None:
    ...
    # pre-v5 documents wrote those ids as raw strings
...
def _index_ambiguous(
    ...
    - ``ambiguous_out``: for each caller, the names it called
      ...
      already discloses via ``ambiguous_in``.
...
def index_matches_disk(root: Path, index: MapIndex) -> bool:
    ...
    rebuilding it after an upgrade is the case that matters), a cache validated
    by ``check_freshness`` alone keeps answering from the old extractor's index
    indefinitely. One ``stat`` closes that. A same-content rewrite costs one
    reload.
...
def _freshness_from_provenance(root: Path, prov: dict) -> Freshness:
    ...
    *this process* is the map the stale party. Otherwise this process
    ...
    (two processes with different specs used to rewrite each other's
    map forever).
    """
